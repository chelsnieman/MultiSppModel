...
Discuss, and maybe include a figure that show how the Biggs model is nested in our model and represents a species parameterization of our model. Our model is slightly more general/flexible because we allow for harvest on both species. *I can make a figure that shows how we can reproduce Bigg's results* *I am not super sure that we need it? Let's keep it in mind, but I think that if we just illustrate our model's behavior that might be enough for this paper?*

This model shows the presence of alternative stable states. These states are driven by cultivation-depensation throuh **XX** (Figure 1). When initial abundance of species 1 is high and species 2 is low, **XX** stable state occurs. However, when we alternate these, we find an alternative stable state in which **XX** which occurs at higher levels of harvest. 
...
```{r, echo=F,cache=T, fig.cap='Figure 2. Temporal influence of management interventions'}
#### figure 1 ####
...
df$outcome=ifelse(df$A1>=200,"darkgreen", "darkred")
vz=ggplot(data = df)+theme_classic()
...
df2$outcome=ifelse(df2$A1>=200,"darkgreen", "darkred")
...
for(i in 1:nrow(dfwo)){
  ...
  st1Fun=approxfun(x=tstep,y=rep(dfwo$Y[i],length(tstep)))
  st2Fun=approxfun(x=tstep,y=rep(0,length(tstep)))
  p=c(s1=0.1,cJ1A1=0.004,cJ1A2=0.001,cJ1J2=0.001,v1=1,f1=4,
...
dfwo$outcome=ifelse(dfwo$A1>=200, "darkgreen", "darkred")
vzwo=ggplot(data = dfwo)+theme_classic()
...
dfwo2$outcome=ifelse(dfwo2$A1>=200, "darkgreen", "darkred")
vzwo=ggplot(data = dfwo2)+theme_classic()
d=vzwo+geom_tile(aes(x=dfwo2$X, y=dfwo2$Y), fill=dfwo2$outcome)+
  scale_fill_gradient(low ="blue", high = "yellow", name="")+
  labs(x="Harvest Rate", y="Stocked fish", title = "No Hysteresis Present")

#comparing with and without hysteresis plots together using ggpubr package
